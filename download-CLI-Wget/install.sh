...
# This function validate if the Wget exists,if yes it returns 0 if no it returns 1
wgetExists() {
...
main() {
 ...
 # Validate if Wget exists before using it.
wgetExists
exit_code=$?

if [ $exit_code -eq 0 ]; then
    echo "wget is installed. Proceeding with download..."
    main
else
    echo "Unable to find wget. Please install wget to proceed."
fi